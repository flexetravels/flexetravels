...
export async function POST(req: Request) {
  ...
  let verifiedRequestedPriceCents = requestedPriceCents;
  let verifiedStripeAmountCents: number | undefined;
  let verifiedStripeCurrency: string | undefined;
  let verifiedStripeBookingReference: string | undefined;
  let verifiedServiceFeeCents: number | undefined;
  let verifiedServiceFeeTaxCents: number | undefined;
  ...
  try {
    if (stripeKey && paymentIntentId) {
      try {
        ...
        // Do not allow the client to add, remove, or swap a flight offer after
        // the PaymentIntent has been created.
        const piOfferId = pi.metadata?.flight_offer_id;
        const hasPaidFlightOffer = !!piOfferId;
        const hasRequestedFlightOffer = !!flightOfferId;
        if (
          hasPaidFlightOffer !== hasRequestedFlightOffer ||
          (piOfferId && flightOfferId && piOfferId !== flightOfferId)
        ) {
          console.error(
        ...
        const piFlightPrice = pi.metadata?.flight_price_cents;
        if (piFlightPrice !== undefined) {
          const parsedPrice = parseInt(piFlightPrice, 10);
          if (!Number.isFinite(parsedPrice) || parsedPrice < 0) {
            console.error('[book-trip] Invalid PI flight_price_cents metadata:', piFlightPrice);
            return NextResponse.json(
              { success: false, error: 'Payment verification failed' },
              { status: 402 },
            );
          }
          verifiedRequestedPriceCents = parsedPrice;
        }
        verifiedStripeAmountCents = pi.amount;
        verifiedStripeCurrency = pi.currency;
        verifiedStripeBookingReference = pi.metadata?.booking_reference;
        verifiedServiceFeeCents = pi.metadata?.service_fee_cents ? parseInt(pi.metadata.service_fee_cents, 10) : undefined;
        verifiedServiceFeeTaxCents = pi.metadata?.service_fee_tax_cents ? parseInt(pi.metadata.service_fee_tax_cents, 10) : undefined;
    ...
    const result = await book({
      ...
      requestedPriceCents: verifiedRequestedPriceCents,
      flightOrigin,
      flightDestination,
      flightDepartureDate,
      flightCabinClass,
      ...
      serviceFeeCents: verifiedServiceFeeCents,
      serviceFeeTaxCents: verifiedServiceFeeTaxCents,
      serviceFeeTaxLabel: verifiedServiceFeeTaxLabel,
      serviceFeeTaxRateBps: verifiedServiceFeeTaxRateBps,
      serviceFeeTaxJurisdiction: verifiedServiceFeeTaxJurisdiction,
    });
    ...
    if (!result.ok) {
      ...
      if (stripeKey && paymentIntentId) {
        ...
        try {
          ...
          console.warn('[book-trip] Booking failed after payment — Stripe refund created:', refund.id, refund.status);
        } catch (e) {
          refundError = String(e);
          console.error('[book-trip] Booking failed after payment and refund attempt failed:', e);
        }
      }
      return NextResponse.json(
        {
    ...
    return NextResponse.json({
      ...
      serviceFeeTaxCents:   d.serviceFeeTaxCents,
      serviceFeeTaxLabel:   d.serviceFeeTaxLabel,
      tripId:               d.tripId,
