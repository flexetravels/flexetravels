...
## What This Is
Travel booking platform with a simple Flights/Hotels booking funnel as the primary customer path. Users search live flights through Duffel or hotels through LiteAPI, compare transparent provider rates, then check out with one visible $20 FlexeTravels service fee plus applicable tax on that fee. The AI chat route remains available as a secondary legacy path.
...
```
app/
  page.tsx              — Homepage booking funnel (Flights/Hotels tabs, live search, filters)
  chat/page.tsx         — Secondary legacy AI chat interface
  ...
  api/
    search/flights/     — Direct Duffel-backed flight search
    search/hotels/      — Direct LiteAPI-backed hotel search
    chat/route.ts       — Secondary AI endpoint (Claude claude-sonnet-4-6, streamText)
    ...
    stripe/prepare/     — Verifies selected Duffel offer and creates PaymentIntent for fare + fee + tax
