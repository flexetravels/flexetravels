...
app/
  ...
  api/
    chat/route.ts                  — Claude streaming endpoint (chat mode + side-channel cards)
    ...
    trip/[id]/command/route.ts     — Cmd+K + ChatPanel — Claude tool-uses to mutate canvas
...
### Claude (AI agent)
- Model: `claude-sonnet-4-6` for both chat and canvas command. `maxSteps: 25` for canvas (multi-leg planning), `maxSteps: 2` for chat.
- Tool surfaces:
...
- `geminiDestinationGuide()` in `lib/ai/gemini.ts` returns a 150-word travel guide. **Note:** despite the name, this currently calls Claude Haiku (the Gemini SDK was swapped in April 2026). Env var name kept as `GEMINI_API_KEY` for legacy reasons.
...
GEMINI_API_KEY=…                 (currently used for the Claude-Haiku destination-guide helper)
...
3. **Vibe sheet** appears once (when `meta.createdFrom === 'new'` and no interests yet). Selection is persisted to `state.meta.interests` and injected into Claude's system prompt for biased picks.
...
- Does not replace Claude with Gemini — they're comparable; cost (Gemini cheaper) vs reliability (Claude more consistent on chained tool calls). Run an A/B before committing.
